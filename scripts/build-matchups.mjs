...
const factionRules = {
  ...
  'Chaos Space Marines': ['chaos-space-marines', 'A flexible combined-arms faction that can mix durable infantry, fast melee, transports and dangerous shooting.', 'Without the submitted list, the exact plan is unknown; expect layered trading pieces and powerful character-led units.', 'Keep the rating provisional. Identify Precision, mortal-wound and D3+ weapons at the table, then decide whether to receive melee or stage for a shooting army.'],
...
const profiles = {
  'Dan Freeman': [4, 'Fast melee is a good structural fit, but the missing list makes the exact threat package unknown.', 'Screen Scout/pre-game lanes, offer Wraiths as the first contact and keep Nightbringer, Skorpekhs and Nekrosor together for the counter-charge.'],
  'Harley Stevens': [4, 'Death Guard are usually slow enough to catch and often rely on damage profiles that Necrodermis handles efficiently.', 'Choose one flank, avoid being debuffed across the whole army and use the Wraiths to hold the centre while the missile deletes one block at a time.'],
  'Luke Vincent': [2, 'GSC mobility, reserve play and short-range shooting can pull the slow Necron package apart.', 'Screen the backfield, deny easy landing zones near Technomancers and force the ambush units to appear within a reliable Wraith counter-charge.'],
  'Tim Anderson': [3, 'Unknown Space Marines are neutral until their mix of mobile shooting, melta and Precision is known.', 'At the table, inventory D3+ guns and sniper effects before deployment; only then decide whether the Wraiths can stage aggressively.'],
  ...
  'Brando McCready': [2, 'The submitted text contains no usable army list, so this is only a Votann faction-level warning about efficient high-damage shooting.', 'Treat the rating as provisional; inspect the actual roster for Hekatons, rail weapons, Thunderkyn and Precision before choosing C’tan lanes.'],
  'Max Pringle': [3, 'CSM can be a favourable melee brawl or a difficult mobile shooting list; no submission exists to distinguish them.', 'Identify the real hammers and Precision at the table, then keep the missile compact and force the opponent to commit through Wraiths.'],
  'Rob Lebeau': [2, 'Drukhari speed and trading can evade the slow counter-charge and open unexpected angles onto Technomancers.', 'Castle the support pieces, cover both flanks with Wraith threat and make the objectives force transports into charge range before exposing the missile.'],
  'Sam Cordell': [1, 'Even without a list, T’au are exactly the fast shooting archetype Kaashif dislikes.', 'Deploy for zero early lines, preserve both Wraith bricks and accept scoring pressure until a multi-tag charge can trap the shooting assets.'],
  ...
  'Chris Shaw': [4, 'Unknown Orks are still likely to bring pressure into the counter-charge zone, but the absence of a list lowers confidence.', 'Check for Scout, transports and character Precision, then screen the first wave and keep the missile together behind the Wraiths.'],
  'Christopher Dyas': [1, 'Unknown T’au remain a bad structural matchup because mobile guns can avoid the slow melee package.', 'Deploy without visible C’tan or Technomancers, then force the actual list to contest several objectives inside Wraith charge distance.'],
  'Liam Macindoe': [1, 'Unknown Thousand Sons still imply dangerous mortal wounds, psychic Devastating Wounds and teleport play.', 'Confirm the actual cabal package, screen landing zones and never rely on Necrodermis alone against direct mortal or Devastating Wound output.'],
  'Nick Watkins': [3, 'Unknown Space Marines could be manageable melee or difficult mobile shooting, so a neutral rating is the honest call.', 'Inventory Precision and D3+ shooting before deployment; hide support pieces and make the first Wraith move respond to the actual threat mix.'],
  ...
  'William Samms': [3, 'The Blood Angels melee is welcome, but two Vindicators, speeders and haywire add serious high-damage shooting and mortal wounds.', 'Hide C’tan from Vindicator lanes, protect Technomancers from the Scout sniper and counter-charge jump units only after the shooting pieces are pinned or screened.'],
};
